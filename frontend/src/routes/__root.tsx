...
export const Route = createRootRouteWithContext<{ queryClient: QueryClient }>()({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Automate moving-quote negotiations and pick the best deal." },
      { property: "og:title", content: "The Negotiator" },
      { property: "og:description", content: "Automate moving-quote negotiations and pick the best deal." },
...
function TopNav() {
  ...
  const active = { className: `${linkBase} !text-primary !bg-accent` };
  return (
    <header className="border-b border-border bg-card">
      <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-3">
        ...
        <nav className="flex items-center gap-1">
          ...
          <Link to="/calls" className={linkBase} activeProps={active}>
            Live Calls
          </Link>
          <Link to="/report" className={linkBase} activeProps={active}>
            Report
          </Link>
        </nav>
      </div>
